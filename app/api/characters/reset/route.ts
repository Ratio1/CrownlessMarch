import { resetCharacterForAccount, AccountServiceError } from '../../../../src/server/auth/account-service';
...
export async function POST(request: Request) {
  ...
  try {
    const account = await resetCharacterForAccount({
    ...
    return Response.json({ error: 'Failed to reset character.' }, { status: 500 });
  }
}
