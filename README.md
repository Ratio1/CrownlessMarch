...
```bash
pnpm live:devnet -- --expect-version=1.10.3
pnpm live:browser -- --expect-version=1.10.3
pnpm live:browser -- --expect-version=1.10.3 --profile=all --combat --report-path=test-results/live/browser-smoke-report.json
```
...
pnpm regression:live -- --expect-version=1.10.3
